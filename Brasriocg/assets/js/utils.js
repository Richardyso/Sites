...
class ParedeCalculator {
    ...
    calcularMateriais() {
        ...
        // Massa para acabamento (vêm em kg - código 698 para 5kg, 431 para 25kg)
        const massaNecessaria = calculateMassa(area, sistema);
        if (massaNecessaria <= 5) {
            addMaterialByCode("698", 1, materiaisSelecionados); // 5kg
        } else {
            const qtd25kg = Math.ceil(massaNecessaria / 25);
            addMaterialByCode("431", qtd25kg, materiaisSelecionados); // 25kg
        }
...
class CimenticiaCalculator {
    ...
    calcularMateriais() {
        ...
        if (this.sistema === 'teto') {
            ...
            // Perfis Steel Frame para parede
            // Guia: perímetro (topo e base) = comprimento * 2, dividido por 3m (tamanho da barra)
            const quantidadeGuias = Math.ceil((this.comprimento * 2) / 3);
            addMaterialByCode("204", quantidadeGuias, materiaisSelecionados); // GUIA 90 STEEL FRAME *0,95* C/ 3.00 MTS
            
            // Montante: espaçados a cada 0,6m ao longo do comprimento
            const quantidadeMontantes = Math.ceil(this.comprimento / 0.6);
            addMaterialByCode("203", quantidadeMontantes, materiaisSelecionados); // MONTANTE 90 STEEL FRAME * 0,95* C/ 3,00 MTS
        }
...
class ForroMineralCalculator {
    ...
    obterAreaPorPlaca(codigoForro) {
        const areas = {
            "161": 9,       // FORRO MINERAL ECOMIN 1250 X 625 X 13MM (12UN) - cobre 9m²
            "194": 7,       // FORRO MINERAL ECOMIN 625 X 625 (18UN) - cobre 7m²
            "601": 5,       // FORRO MINERAL FEINSTRATOS (NRC 0,60) TEGULAR 625X625 C/14 PÇ - cobre 5m²
            "867": 7        // FORRO MINERAL FEINSTRATOS (NRC 0,60) 1250 X 625 X 15MM C/10 PÇ - cobre 7m²
        };
        return areas[codigoForro] || 7; // Default para 7m² se não encontrar
    }
...
async function carregarProdutos() {
  try {
    ...
    alert('Erro ao carregar a lista de produtos. Verifique se o arquivo ../assets/data/produtos.json está disponível.');
  }
}
...
function calcularMateriais(material, subtype, m2, placaSel, quantidadeJanelas = 0, quantidadePortas = 0, peDireito = null) {
  ...
  if (material === "Cimenticia") {
    ...
    if (subtype === "Teto") {
      ...
      addMaterialByCode("32", Math.ceil(m2 * 1.35), materiaisSelecionados); // Regulador F530 (fórmula: área × 1,35 arredondado para cima)
    } else {
      // Perfis para parede
    ...
    // Forro isopor: cálculo inteligente com pacotes + placas avulsas
    const areaPackote = 19; // Cada pacote [68] cobre 19m²
    const areaPlacaAvulsa = 0.768; // Cada placa [365] cobre 1.243m × 0.618m = 0.768m²
    
    // Calcula quantos pacotes completos usar
    const pacotesCompletos = Math.floor(m2 / areaPackote);
    const areaRestante = m2 - (pacotesCompletos * areaPackote);
    
    // Se a área restante for muito pequena (< 10% de um pacote), usa o pacote completo
    // Caso contrário, usa placas avulsas para o restante
    if (areaRestante > 0) {
      if (areaRestante < (areaPackote * 0.1)) {
        // Área muito pequena, adiciona 1 pacote completo
        addMaterialByCode("68", pacotesCompletos + 1, materiaisSelecionados); // Forro isopor pacote
      } else {
        // Adiciona os pacotes completos
        if (pacotesCompletos > 0) {
          addMaterialByCode("68", pacotesCompletos, materiaisSelecionados); // Forro isopor pacote
        }
        // Adiciona placas avulsas para o restante
        const placasAvulsas = Math.ceil(areaRestante / areaPlacaAvulsa);
        addMaterialByCode("365", placasAvulsas, materiaisSelecionados); // Forro isopor placa avulsa
      }
    } else {
      // Área exata de pacotes completos
      addMaterialByCode("68", pacotesCompletos, materiaisSelecionados); // Forro isopor pacote
    }
    ...
    // Telha ondulada: cada unidade cobre 2.44m x 1.10m = 2.684m²
    const areaPorTelha = 2.684; // 2.44m x 1.10m
    const quantidadeTelhas = Math.ceil(m2 / areaPorTelha);
    addMaterialByCode("1613", quantidadeTelhas, materiaisSelecionados); // TELHA ONDULADA 2.44 X 1.10 5MM CASALITE
  }
